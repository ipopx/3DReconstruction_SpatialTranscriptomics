...
from utils.data_loader import load_merfish_dataset
    ...
    *,
    ...
    if model_name == "uot":
        return load_or_generate_uot_slice(
            ...
            save_name_prefix=data_cfg.get("save_name_prefix", "merfish"),
            uot_cfg=uot_cfg,
            data_cfg=data_cfg,
        )
    raise FileNotFoundError(
        f"Missing simulated slice for model {model_name!r}: {full_path}. "
        f"Run evaluation_merfish_small.py {model_name} first."
    )
...
def main():
    ...
    compare_cfg = config.get("compare_merfish", config.get("compare", {}))
    ...
    for i, slice_id in enumerate(compare_cfg.get("slices_to_compare")):
        ...
        plot_save_path = (
            Path(compare_cfg.get("output_dir", spatialz_cfg.get("output_dir", "output"))) / "compare"
            / f"merfish_{model_names[0]}_vs_{model_names[1]}_slice_{slice_id}.png"
        )
