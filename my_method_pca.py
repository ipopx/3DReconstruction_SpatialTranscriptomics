...
def generate_gex_pca(
    ...
    start_time = time.time()
    if verbose:
        print("Begin to synthesize gene expression with PCA......")

    def _to_dense_rows(X, row_idx: np.ndarray) -> np.ndarray:
        X_sub = X[row_idx]
        if hasattr(X_sub, "A"):  # sparse matrix
            return X_sub.A
    ...
    # Fit PCA on left+right slice expression (unless a fitted model is provided)
    if pca_model is None:
        X1 = adata1.X
        X2 = adata2.X
        X1d = X1.A if hasattr(X1, "A") else np.asarray(X1)
        X2d = X2.A if hasattr(X2, "A") else np.asarray(X2)
        X_fit = np.vstack([X1d, X2d]).astype(np.float32, copy=False)
        pca_model = PCA(n_components=n_components, svd_solver="auto", random_state=0)
        pca_model.fit(X_fit)
    ...
    for i in range(n_cells):
        ...
        # Gather neighbour expression rows from adata1/adata2
        nb_rows = []
        for o in order:
            src, idx = neighbor_sources[int(o)]
            if src == "adata1":
                nb_rows.append(_to_dense_rows(adata1.X, np.array([idx], dtype=int))[0])
            else:
                nb_rows.append(_to_dense_rows(adata2.X, np.array([idx], dtype=int))[0])
        X_nb = np.vstack(nb_rows).astype(np.float32, copy=False)  # (k, n_genes)

        # Encode with PCA -> average -> decode with inverse PCA
        Z_nb = pca_model.transform(X_nb)  # (k, n_components)
        z_mean = Z_nb.mean(axis=0, keepdims=True)  # (1, n_components)
        x_rec = pca_model.inverse_transform(z_mean).reshape(-1)
        X_new[i, :] = x_rec.astype(np.float32, copy=False)

    adata3.X = X_new

    if time_logger is not None:
        time_logger("Gene expression synthesis with PCA", start_time)
    elif verbose:
        print(
            f"Gene expression synthesis with PCA time: {time.time() - start_time:.2f} seconds"
        )

    return adata3


def generate_pca(adata1, adata2, adata1_id='above', adata2_id='below',
