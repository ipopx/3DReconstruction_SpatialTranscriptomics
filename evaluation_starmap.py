...
def main():
    ...
    for i, test_slice in enumerate(test_slices):  
        ...
        alpha = middle_slice.obs['slice_id'].values[0]/ (right_slice.obs['slice_id'].values[0] +  left_slice.obs['slice_id'].values[0])
        ...
        if model_to_run == "spatialz":
            sim_middle_slice = Generate_spatialz(adata1=left_slice, adata2=right_slice, adata1_id='above', adata2_id='below',
                        alpha=alpha, device=device, n_cell=n_cell_middle_slice, k_neighbors=k_ct,
            ...
            sim_middle_slice = generate_pca(adata1=left_slice, adata2=right_slice, adata1_id='above', adata2_id='below',
                        alpha=alpha, device=device, n_cell=n_cell_middle_slice, k_ct=k_ct,
                        n_mag=1.0, lr=1e5, nb_iter_max=nb_iter_max, seed=42,
                        num_projections=80, cell_type_key=data_cfg.get("cell_class_key", "leiden"), k_gex=k_gex, add_obs_list= None, 
                        verbose=spatialz_cfg.get("verbose"))
